...
@register_meta_arch("MaxAvg_net")
class MaxAvg_net(nn.Module):
    """
    ...
    def __init__(
        ...
        assert backbone_type in ['avg_max']
        if backbone_type == 'avg_max':
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        if gt_segment is None:
            cls_targets = torch.zeros(num_pts, self.num_classes, dtype=torch.float32)
            cls_targets = cls_targets.to(self.device)
            reg_targets = torch.zeros(num_pts, 2, dtype=torch.float32)
    ...
    def losses(
        ...
        # focal loss
        # print(torch.unique(gt_cls), flush=True)
        # print(gt_cls.shape)
        # print(torch.unique(gt_target))
    ...
    def inference(
        ...
        # prob = 0.005246319442561613
        # new_out_cls_logits = []
        # for out_cls_logit in out_cls_logits:
        #     probs = torch.ones_like(out_cls_logit) * prob
        #     new_out_cls_logit = out_cls_logit + torch.log(probs)
        #     new_out_cls_logits.append(new_out_cls_logit)
        # out_cls_logits = new_out_cls_logits
