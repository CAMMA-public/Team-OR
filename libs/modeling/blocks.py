...
class TemporalMaxAvger(nn.Module):
    def __init__(
            self,
            kernel_size,
            stride,
            ...
            n_embd):
        super().__init__()
        self.ds_pooling_max = nn.MaxPool1d(
            kernel_size, stride=stride, padding=padding)
        self.ds_pooling_avg = nn.AvgPool1d(
           kernel_size, stride=stride, padding=padding)

        self.stride = stride

    def forward(self, x, mask, mode='max', **kwargs):
        if self.stride > 1:
            # downsample the mask using nearest neighbor
            out_mask = F.interpolate(
        ...
        if mode == 'max':
            out = self.ds_pooling_max(x) * out_mask.to(x.dtype)
        elif mode == 'avg':
            out = self.ds_pooling_avg(x) * out_mask.to(x.dtype)
        else:
            raise ValueError('Mode ' + mode + ' is not implemented! ')
