...
def load_gt_seg_from_json(cfg, split=None, label='label_id', label_offset=0):
    json_file = cfg['dataset']['json_file']

    label_dict = {}
    label_dict[cfg['dataset']['seg_name']] = 0
    
    vid_fea_dir = os.path.join(cfg['dataset']['feat_folder'], 'videomaev2')
    ...
    if split == 'training':
        vid_list = vid_names[:int(vid_num * cfg['dataset']['train_set_ratio'])]
    else:
        vid_list = vid_names[int(vid_num * cfg['dataset']['train_set_ratio']):]
    ...
    for key, value in anno_dict.items():
        ...
        for n in range(len(value)):
            ...
            if value[n]['label'] in label_dict:
                if cfg['dataset']['default_fps'] is not None:
                    fps = cfg['dataset']['default_fps']
                elif 'fps' in value[n]:
                    fps = value[n]['fps']
                else:
                ...
                vids += [vid_name]
                starts += [float(start_frame / fps)]
                stops += [float(end_frame / fps)]
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        ant_file = cfg['dataset']['json_file']
        ...
        self.ground_truth = load_gt_seg_from_json(
            cfg, split=self.split, label=label, label_offset=label_offset)
